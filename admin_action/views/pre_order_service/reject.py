...
class AdminRejectPreOrderService(GenericAPIView):
    ...
    def post(self, request, ord_id):
        ...
        admin_accept_or_reject(False, None, request.user.admin_profile,
                               cnt, service.id, request.data['comment'])

        return Response({'status': 'rejected pre order service'})
